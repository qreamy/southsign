...
export async function proxy(request: NextRequest){
  ...
  if(request.nextUrl.pathname.startsWith('/dashboard')&&!user){const u=request.nextUrl.clone();u.pathname='/auth/login';return NextResponse.redirect(u)}
  if(request.nextUrl.pathname==='/auth/login'&&user){const u=request.nextUrl.clone();u.pathname='/dashboard';return NextResponse.redirect(u)}
  return response;
}
